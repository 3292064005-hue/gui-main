...
std::string SdkRobotFacade::sdkBindingMode() const {
  if (liveTakeoverReady()) return "live_takeover_ready";
  if (vendored_sdk_detected_ && live_binding_established_) return "live_binding_degraded";
  if (vendored_sdk_detected_) return control_source_exclusive_ ? "vendored_sdk_contract_shell" : "vendored_sdk_nonexclusive";
...
std::string SdkRobotFacade::runtimeSource() const {
  if (liveTakeoverReady()) return xmateModelAvailable() ? "xcore_sdk_live_takeover+xmatemodel" : "xcore_sdk_live_takeover";
  if (vendored_sdk_detected_ && xmateModelAvailable()) return "xcore_sdk_contract_shell+xmatemodel";
  if (vendored_sdk_detected_) return "xcore_sdk_contract_shell";
