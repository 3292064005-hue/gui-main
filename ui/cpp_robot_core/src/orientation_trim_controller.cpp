...
void OrientationTrimController::configure(const OrientationTrimConfig& config) {
  config_ = config;
  config_.gain = std::max(0.0, config.gain);
  config_.max_trim_deg = std::max(0.1, config.max_trim_deg);
  config_.lowpass_hz = std::max(0.1, config.lowpass_hz);
}

OrientationTrimState OrientationTrimController::step(double force_error_n, double dt_s) {
  const double target = std::clamp(config_.gain * force_error_n, -degToRad(config_.max_trim_deg), degToRad(config_.max_trim_deg));
  const double alpha = std::clamp(dt_s * 2.0 * M_PI * config_.lowpass_hz, 0.0, 1.0);
  state_.trim_rad += alpha * (target - state_.trim_rad);
  const double max_trim = degToRad(config_.max_trim_deg);
