...
def test_runtime_command_capability_claims_cover_rt_motion_mainline() -> None:
    ...
    assert {'approach_prescan', 'seek_contact', 'start_procedure', 'pause_scan', 'resume_scan', 'stop_scan', 'safe_retreat'} <= set(claims['rt_motion_write'])
    assert command_capability_claim('validate_scan_plan') == 'plan_compile'
    assert command_capability_claim('compile_scan_plan') == ''


def test_runtime_command_alias_metadata_is_not_active_surface() -> None:
