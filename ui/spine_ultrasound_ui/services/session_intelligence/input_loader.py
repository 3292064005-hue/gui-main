...
from spine_ultrasound_ui.services.session_intelligence.product_contracts import SessionIntelligenceInputBundle


class SessionIntelligenceInputLoader:
    """Load persisted session artifacts needed for intelligence products."""

    def load(self, service, session_dir: Path) -> SessionIntelligenceInputBundle:
        manifest = service._read_json(session_dir / "meta" / "manifest.json")
        return SessionIntelligenceInputBundle(
            manifest=manifest,
