...
class WorkflowContext:
    ...
    localization_review_required: bool = False
    localization_review_approved: bool = False
...
class WorkflowStateMachine:
    ...
    def permission_matrix(self, ctx: WorkflowContext) -> dict[str, dict[str, str | bool]]:
        ...
        return {
            ...
            "generate_path": self._rule(
                ctx.has_experiment and ctx.localization_ready and ready_for_local_ops and (not ctx.localization_review_required or ctx.localization_review_approved),
                ok_reason="定位完成且复核条件满足，可生成扫查路径。",
                blocked_reason=self._path_reason(ctx, ready_for_local_ops),
            ),
            "approve_localization_review": self._rule(
    ...
    def _path_reason(ctx: WorkflowContext, ready_for_local_ops: bool) -> str:
        ...
        if not ctx.localization_ready:
            return "需要先完成视觉定位。"
        if ctx.localization_review_required and not ctx.localization_review_approved:
            return "当前视觉引导结果需要人工复核，批准后才能生成扫查路径。"
        if not ready_for_local_ops:
            return "需要系统处于允许的本地规划状态。"
        return "当前不可生成扫查路径。"

    @staticmethod
    def _review_reason(ctx: WorkflowContext, ready_for_local_ops: bool) -> str:
        if not ctx.has_experiment:
            return "需要先创建实验。"
        if not ctx.localization_review_required:
            return "当前没有待批准的视觉引导复核。"
        if ctx.localization_review_approved:
            return "当前视觉引导复核已批准。"
        if not ready_for_local_ops:
            return "需要系统处于 AUTO_READY / SESSION_LOCKED / PATH_VALIDATED / SCAN_COMPLETE。"
        return "当前不可执行视觉引导复核批准。"
