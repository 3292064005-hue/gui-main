...
class ConfigForm(QGroupBox):
    ...
    def __init__(self, config: RuntimeConfig):
        ...
        self._mainline = ClinicalConfigService()
        ...
        self.rt_mode.addItems([self._mainline.identity.rt_mode])
        self.preferred_link = QComboBox()
        self.preferred_link.addItems([self._mainline.identity.preferred_link])
        self.robot_model = QComboBox()
        self.robot_model.addItems([self._mainline.identity.robot_model])
        self.sdk_robot_class = QComboBox()
        self.sdk_robot_class.addItems([self._mainline.identity.sdk_robot_class])
        self.axis_count = QComboBox()
        self.axis_count.addItems([str(self._mainline.identity.axis_count)])
        self.requires_single_control_source = QComboBox()
        self.requires_single_control_source.addItems(['true' if self._mainline.identity.requires_single_control_source else 'false'])
        ...
        if hasattr(self.apply_btn, 'setProperty'):
            self.apply_btn.setProperty('kind', 'primary')
        self.apply_btn.clicked.connect(self._emit_config)
        layout.addRow(self.apply_btn)
        self._lock_mainline_controls()
        self.populate_from_config(config)

    def _lock_mainline_controls(self) -> None:
        for widget in (
            self.rt_mode,
            self.preferred_link,
            self.robot_model,
            self.sdk_robot_class,
            self.axis_count,
            self.requires_single_control_source,
        ):
    ...
    def _set_widget_enabled(widget, enabled: bool) -> None:
        """Set enabled state across full Qt and minimal widget shims used in tests."""
        if hasattr(widget, 'setEnabled'):
            widget.setEnabled(enabled)
        setattr(widget, '_enabled_state', enabled)
        if not hasattr(widget, 'isEnabled'):
            widget.isEnabled = lambda w=widget: getattr(w, '_enabled_state', True)

    @staticmethod
    def _set_combo_value(widget: QComboBox, value: str) -> None:
        """Best-effort combo assignment across full Qt and lightweight test stubs."""
    ...
    def populate_from_config(self, config: RuntimeConfig) -> None:
        ...
        self._set_combo_value(self.roi_mode, config.roi_mode)
        self._set_combo_value(self.rt_mode, self._mainline.identity.rt_mode)
        ...
        self._set_combo_value(self.preferred_link, self._mainline.identity.preferred_link)
        self._set_combo_value(self.robot_model, self._mainline.identity.robot_model)
        self._set_combo_value(self.sdk_robot_class, self._mainline.identity.sdk_robot_class)
        self._set_combo_value(self.axis_count, str(self._mainline.identity.axis_count))
        self._set_combo_value(self.requires_single_control_source, 'true' if self._mainline.identity.requires_single_control_source else 'false')
        ...
        self._set_combo_value(self.collision_enabled, 'true' if config.collision_detection_enabled else 'false')
        self.collision_sensitivity.setText(str(config.collision_sensitivity))
        self._set_combo_value(self.collision_behavior, config.collision_behavior)
        self.collision_fallback_mm.setText(str(config.collision_fallback_mm))
        self._set_combo_value(self.soft_limit_enabled, 'true' if config.soft_limit_enabled else 'false')
        self.soft_limit_margin_deg.setText(str(config.joint_soft_limit_margin_deg))
        self._set_combo_value(self.singularity_avoidance_enabled, 'true' if config.singularity_avoidance_enabled else 'false')
        self.rl_project_name.setText(config.rl_project_name)
        self.rl_task_name.setText(config.rl_task_name)
        self._set_combo_value(self.xpanel_vout_mode, config.xpanel_vout_mode)
    ...
    def read_config(self) -> RuntimeConfig:
        ...
        payload.update(
            ...
            rt_mode=self._mainline.identity.rt_mode,
            ...
            preferred_link=self._mainline.identity.preferred_link,
            robot_model=self._mainline.identity.robot_model,
            sdk_robot_class=self._mainline.identity.sdk_robot_class,
            axis_count=int(self._mainline.identity.axis_count),
            requires_single_control_source=bool(self._mainline.identity.requires_single_control_source),
