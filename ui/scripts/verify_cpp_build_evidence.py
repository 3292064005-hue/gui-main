...
def configure(repo_root: Path, build_dir: Path, profile: str, with_sdk: bool, with_model: bool) -> subprocess.CompletedProcess[str]:
    cmd = ['cmake', '-S', 'cpp_robot_core', '-B', str(build_dir), '-DCMAKE_EXPORT_COMPILE_COMMANDS=ON', f'-DROBOT_CORE_PROFILE={profile}', f'-DROBOT_CORE_WITH_XCORE_SDK={'ON' if with_sdk else 'OFF'}', f'-DROBOT_CORE_WITH_XMATE_MODEL={'ON' if with_model else 'OFF'}']
...
def build_targets(repo_root: Path, build_dir: Path) -> dict[str, str]:
    results: dict[str, str] = {}
    for target in KEY_TARGETS:
        proc = run(['cmake', '--build', str(build_dir), '--target', target, '-j1'], repo_root)
        results[target] = 'ok' if proc.returncode == 0 else (proc.stderr or proc.stdout)[-500:]
...
def main() -> int:
    ...
    try:
        ...
        if configure_proc.returncode != 0:
            ...
            report['target_results'] = build_targets(repo_root, build_dir)
    ...
    return 0 if (report.get('configure_returncode') == 0 and (target_ok or syntax_ok)) else 1
